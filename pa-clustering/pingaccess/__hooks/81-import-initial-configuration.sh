...
while true; do
  curl -ss --silent -o /dev/null -k https://morgan.ping-eng.com:9000/pa/heartbeat.ping 
  if ! test $? -eq 0 ; then
...
}' https://morgan.ping-eng.com:9000/pa-admin-api/v3/users/1 > /dev/null
...
}' https://morgan.ping-eng.com:9000/pa-admin-api/v3/users/1/password > /dev/null
...
curl -k -v -X POST -u Administrator:${INITIAL_ADMIN_PASSWORD} -H "Content-Type: application/json" -H "X-Xsrf-Header: PingAccess" \
  ...
  https://morgan.ping-eng.com:9000/pa-admin-api/v3/config/import

echo "apps after import"
curl -k -u Administrator:${INITIAL_ADMIN_PASSWORD} -H "X-Xsrf-Header: PingAccess" https://morgan.ping-eng.com:9000/pa-admin-api/v3/applications